...
class AuthRepository implements AuthRepoInterface {
  ...
  Future<ApiResponse> SendOtp(String phone) async {
    try {
      Response response =
          await dioClient!.post(AppConstants.sendOtp, data: {"phone": phone});
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  //   try {
  //     final response = await dioClient!.post(AppConstants.verifyPhone,
  //         data: {"phone": phone.trim(), "otp": otp});
  //     return response.data; // Assuming response.data is a Map<String, dynamic>
  //   } catch (e) {
  ...
  Future<Map<String, dynamic>> VerifyOTP(String phone, String otp) async {
    ...
    try {
      ...
      return {'error': ApiErrorHandler.getMessage(e)};
    }
  }
}
