...
class ShopController extends ChangeNotifier {
  ...
  Future<ApiResponse> getMoreStore() async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      apiResponse.response?.data.forEach((store) {
        moreStoreList.add(MostPopularStoreModel.fromJson(store));
      });
    } else {
      isLoading = false;
      ApiChecker.checkApi(apiResponse);
  ...
  Future<void> getTopSellerList(bool reload, int offset,
    ...
    if (apiResponse.response != null &&
      ...
      int counter = 1;
      for (var element in sellerModel!.sellers!) {
        String storeModelName = counter.toString().padLeft(6, '0');
        element.shop!.name = "مزرعة $storeModelName ";
        element.shop!.imageFullUrl!.path = Images.sellerlogo;
        element.shop!.bannerFullUrl!.path = Images.sellerBanner;
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log(
          "storeModelName ${element.shop!.name}",
        );
        log(
          "storeModelImage ${element.shop!.image}",
        );
        log(
          "storeModelImage ${element.shop!.imageFullUrl!.path}",
        );
        log(
          "storeModelBanner ${element.shop!.banner}",
        );
        log(
          "storeModelBanner ${element.shop!.bannerFullUrl!.path}",
        );
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
        log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
